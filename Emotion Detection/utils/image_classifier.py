...
class Dataset:
    def __init__(self, df: pd.DataFrame):
        self.data = df
        # class_feature = last feature
        self.class_feature = df.columns.values[-1:][0]
        # class_feature = features except last feature
...
class ImageClassifier:
    def __init__(self, csv_path: str, algorithm: str, land_marker: LandMarker):
        ...
        if algorithm == 'RandomForest':
            self.classifier = RandomForestClassifier(
                n_jobs=2, random_state=0, n_estimators=100)
    ...
    def classify(self, img: np.ndarray) -> List[str]:
        face_land_marks_list = self.land_marker.img_to_landmarks(
            img, exclude_vector_base=True)
    ...
    def evaluate_with_random_forest(self, split_ratio: float, n_jobs: int = 2, random_state: int = 0):
        a_classifier = RandomForestClassifier(
            n_jobs=n_jobs, random_state=random_state, n_estimators=100)
        ...
        predicted_labels = [test.unique_labels[predict]
                            for predict in a_classifier.predict(X=test.without_labels)]
    ...
    def split_data(self, split_ratio: float) -> tuple:
        ...
        np.random.seed(0)
        data[is_train_feature] = np.random.uniform(
            0, 1, len(data)) <= split_ratio
        train_df, test_df = data[data[is_train_feature]
                                 == True], data[data[is_train_feature] == False]
        del train_df[is_train_feature], test_df[is_train_feature]

        print('[INFO]', 'Size of training data: %d, Size of test data: %d' %
